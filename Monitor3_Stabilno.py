...
def Alarm():
	...
	if SkakaBoom[0]==1 and AlarmaTorika!=0:	
		...
		if time.time()-SkakaBoom[3]>2: OOOh_Jizn_Jestyanka(AlarmaTorika)
def Avremenet(mass):
	x=0
	for y in mass:
		x+=y
		...
		if Kuda!='./':
			...
			if fil.read(3)=='6^6':
				...
				for data in faliant:
					...
					data=data.split(' ')
					try:
						dlyaShkali.append(data[4]+' '+data[5])
					except:
						print(data)
